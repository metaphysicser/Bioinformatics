...
def TraceBack(pm:list, A:str, B:str, k:int, channel:int):
    ...
    return seq_A[::-1], seq_B[::-1], A_gap_loc[::-1], B_gap_loc[::-1]
...
def PSA_AGP_Kband(A: str, B: str, m=1, mis=-2, opengap=3, exgap=1, get_score=0):
    ...
    SeqA, SeqB, A_gap_loc, B_gap_loc = TraceBack([t,x,y], A, B, k, channel)

    # exchange the loc of A & B
    if state_ex:
        SeqA, SeqB = SeqB, SeqA
        A_gap_loc, B_gap_loc = B_gap_loc, A_gap_loc

    return new, SeqA, SeqB, A_gap_loc, B_gap_loc
