...
def NW(s1, s2, match=1, mismatch=-1, gap=-1):
    ...
    for i in range(1, m):
        for j in range(1, n):
            ...
            al_mat[i][j] = max(di, ho,
                               ve)  # Fill the matrix with the maximal value.(based on the python default maximum)
...
class GymEnv:
    ...
    def GetActionSpace(self):
        ...
        for i in range(0, (self.noOfRows * self.noOfCols)):
            if (
                    i % self.noOfCols != 0 and i % self.noOfCols != self.noOfCols - 1):  # first col and last col of each row is ignored
...
def central_agent(net_params_queues, exp_queues):
    ...
    if not os.path.exists(SUMMARY_DIR + '/log_central'):
        os.makedirs(SUMMARY_DIR + '/log_central')
    ...
    with tf.Session() as sess, open(SUMMARY_DIR + '/log_agent_' + str(agent_id), 'wb') as log_file:
        ...
        for ep in range(TRAIN_EPOCH):
            ...
            for step in range(TRAIN_SEQ_LEN):
                ...
                stateBuffer = np.reshape(stateOriginal,
                                         [1, stateOriginal.shape[0], stateOriginal.shape[1], stateOriginal.shape[2]])
            ...
            if not done:
                ...
                log_file.write('seq ' + str(seq) + ' epoch ' + str(ep) + ' reward ' + str(
                    np.sum(r_batch[0:ind + 1])) + ' step ' + str(len(r_batch[0:ind + 1])) + ' AlignmentDiff ' + str(
                    rewardDiff) + ' reachedGoal ' + reachedGoal)
                log_file.write("\n")
                log_file.flush()
